  ...
  module Resource
    ...
    def attributes=(attributes)
      attributes.each do |key, value|
        send("#{key}=", value) if self.class.simple_sdk_attributes.include?(key.to_s)
      end
    end
    ...
    def build_attribute(attr, value, options)
      ...
      if options[:nested] && value.is_a?(Array)
        value.map { |v| build_attribute(attr, v, options) }
    ...
    def guess_class_name(attr, value, polymorphic = false)
      ...
      namespace = class_name[0..class_name.rindex(':')] if class_name.rindex(':')
